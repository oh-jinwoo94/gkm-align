...
\begin{align*}
f(P)= \sum_{q=1}^{|P|} G(P(q)) - \alpha \cdot I,
\end{align*}
...
where
\begin{align*}
     &P_{(+)} = \frac{1}{\sqrt{2\pi \sigma_{(+)}^2}}\exp\left(-\frac{(pred(X)-\mu_{(+)})^2}{2\sigma_{(+)}^2}\right)\\ 
     &P_{(-)} = \frac{1}{\sqrt{2\pi \sigma_{(-)}^2}}\exp\left(-\frac{(pred(X)-\mu_{(-)})^2}{2\sigma_{(-)}^2}\right)
 \end{align*}
$pred(X)$ is computed using Eq.10, summing up kmer-weights of every kmer contained in $X$.
